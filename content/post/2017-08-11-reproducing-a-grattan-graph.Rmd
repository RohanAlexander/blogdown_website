...
# Introduction
The Grattan Institute is an Australian think-tank that produces reports about public policy. Last week they released ['Regional patterns of Australia's economy and population'](https://grattan.edu.au/report/regional-patterns-of-australias-economy-and-population/). That report looks into the differences between geographic areas across various economic and demographic variables. It included interactive maps made using Carto. In addition to the report, the Grattan Institute released the datasets that they said underlay their maps and graphs.

I was interested to see if I could reproduce one of their interactive maps - [Annual income growth per person (FY04 - FY15)](https://grattan.carto.com/me) - in an hour using R and the leaflet package. A day later I found the underlying dataset did not correspond with the map that the Grattan Institute published. But following the methodology in the report I was able to create a dataset that was pretty close to the values illustrated on their map.

The final map that I produced is below and this note records what I did to produce it.
...
## Incomes
The Grattan Institute released the datasets that they said had been mapped. It was straightforward to download this and export the relevant sheet as a csv file. The Excel file is available at https://grattan.edu.au/report/regional-patterns-of-australias-economy-and-population/ (in the left panel).
...
I didn't spend too much time on the colours because the inferno palette got fairly close. Reproducing the breaks that Grattan used was straightforward by copying their bins. Box 1 in Appendix A of the Grattan Institute's report specifies how they came up with these bins. Give my purposes I didn't worry too much about this.
...
The key issue is which is right - the Carto map which is the one in the report or the dataset that they released. 
...
Without their code it's hard to be sure, but the Grattan Institute provided enough information in the report that I was confident I could get reasonably close to what they'd done.
...
### Correspondence
The incomes data that the ATO makes available is on a postcode basis. I needed to convert this into 2011 SA3 levels and the ABS makes a correspondence available for this purpose from: 
...
I think the maps in the Grattan Institute report are broadly fine, and it was just the accompanying dataset that is wrong. My dataset can be downloaded as a csv here:
...
It was fun to spend the day in the data-analyst's version of a treasure chase. But hopefully the next time I decide to reproduce a Grattan Institute graph common sense prevails before I go too far down the rabbit hole.
